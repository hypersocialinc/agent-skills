...
- `ios-clerk-auth`
  Wire and debug Clerk authentication in a native iOS / Swift app. Covers the native Sign in with Apple "You are not authorized to perform this request" failure (empty `connection_oauth_apple.bundle_id` — the field native iOS needs, distinct from the web `client_id`/Services ID), diagnosing and patching Clerk instance config with the `clerk` CLI, the Clerk↔Convex JWT template wiring (`convex` template `aud` ↔ `applicationID` + `CLERK_JWT_ISSUER_DOMAIN`), and using Google as a dev fallback to isolate Apple-only issues.
