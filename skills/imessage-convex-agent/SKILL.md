...
description: Build an iMessage AI agent channel on a Convex backend — Sendblue webhook → Convex brain → agent (OpenRouter via @convex-dev/agent) → replies, typing indicators, card images, account linking. Use when adding "text this number, the agent handles it" to any Convex project.
---

# iMessage agent on Convex (Sendblue)

Battle-tested playbook from savethis (github.com/hypersocialinc/savethis — see
`convex/channels/imessage.ts`, `convex/chat.ts`, `convex/chats.ts`,
`convex/links.ts`, `convex/http.ts` for a working reference implementation).
...
- **@convex-dev/agent** (Convex agent component) + **OpenRouter** as the model
  gateway (AI SDK `generateText` under the hood). For the agent/streaming
  architecture itself (workers, persisted partials, client UX), compose with
  the `convex-streaming-agents` skill — this skill covers the CHANNEL.

## Architecture

```
Sendblue webhook (POST, sb-signing-secret header)
  → convex/http.ts httpAction: verify secret → parse → enqueue
  → chats.enqueueInbound (mutation): dedupe by (channel, updateId),
    upsert chat row, schedule brain
  → chat.processInbound (action): resolve identity → bind ownership →
    typing indicator → ingest media → agent.generateText → send reply
    → send card images for side effects (saves)
```

Key tables: `chats` (channel, chatKey=E.164, userId?, threadId?),
`processedUpdates` (channel+updateId dedupe), `identities` (channel,
channelKey → userId), `linkCodes` (code, userId, expiresAt).
...
- **Webhook secret arrives as the `sb-signing-secret` HEADER** (also accept
  `?secret=` on the registered URL as belt-and-braces). Compare
...
- **ONE webhook registration per number.** Registering dev + prod endpoints
  simultaneously double-replies and double-saves. Point the webhook at ONE
  deployment; test the other by curling simulated webhook payloads at it.
...
the recipient's contacts does NOT fix this (verified). So:
...
App-issued short codes, no SMS verification service needed:
...
- App calls `createLinkCode` (authed mutation) → 6-char code from a
  ...
  via `crypto.getRandomValues`. User texts the code to the line.
...
- The app shows a reactive `textLinkStatus` query so the sheet flips to
  "Linked" the moment the code lands. Deep-link Messages pre-filled:
  `sms:<number>&body=<code>`.
...
## ⚠️ THE ownership trap (cost us a silent total failure)
...
Otherwise every tool write silently no-ops while the agent happily replies
"Saved!" — the reply text proves nothing.
...
- Make a **text-channel variant** of your main agent: same instructions core,
  ...
  lists; add a text-channel addendum to the system prompt.
...
- **Media inbound**: provider URLs are short-lived — fetch to Convex storage
  ...
  plus storage-id hints in the prompt so save tools can persist them.
...
- No typing indicator needed for instant flows (link help/confirm) — fire it
  only before agent runs.
...
4. Text from an UNLINKED number → link-help flow, no agent run.
